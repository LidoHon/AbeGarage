  ...
  useEffect(() => {
    const fetchServices = async () => {
      ...
      try {
        ...
        console.log("Data received from API (service):", JSON.stringify(data, null, 2));
        setServices(Array.isArray(data.services) ? data.services : []); // Ensure it's an array
        setError(null);
      } catch (err) {
  ...
  const handleServiceSelect = (serviceId) => {
    if (selectedServices.includes(serviceId)) {
      setSelectedServices((prev) => {
        const newSelection = prev.filter((id) => id !== serviceId);
        onSelectServices(newSelection); // Notify parent with updated selection
        return newSelection;
      });
    } else {
      setSelectedServices((prev) => {
        const newSelection = [...prev, serviceId];
        onSelectServices(newSelection); // Notify parent with updated selection
        return newSelection;
      });
    }
